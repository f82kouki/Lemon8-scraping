...
def test_end_to_end_success(monkeypatch):
    ...
    assert rows[0]["effective_author_link_name"] == "owner_1"
    assert rows[0]["author_source"] == "html"
...
def test_end_to_end_ownership_fallback_to_final_url(monkeypatch):
    def fake_fetch(url: str, retry_count: int = 3, base_delay_sec: float = 0.7):
        return FetchResult(
            url=url,
            final_url="https://www.lemon8-app.com/@f82_bmw4/7613685322899079688",
            http_status=200,
            ok=True,
            error_type=None,
            raw_html="""
            <script>
            window.__STATE__ = {
              "$ArticleDetail+abc": {"readCount": 24637}
            };
            </script>
            """,
        )
    ...
    targets = [
        ValidationTarget(
            user_id="u1",
            url="https://s.lemon8-app.com/al/GgTURfrjvc",
            region="jp",
            expected_link_names=["f82_bmw4"],
        )
    ]
    rows = run_batch_validation(targets, allowed_regions={"jp"})
    assert rows[0]["read_count"] == 24637
    assert rows[0]["effective_author_link_name"] == "f82_bmw4"
    assert rows[0]["author_source"] == "final_url"
    assert rows[0]["ownership_status"] == "matched"


def test_end_to_end_ownership_fallback_invalid_final_url(monkeypatch):
    def fake_fetch(url: str, retry_count: int = 3, base_delay_sec: float = 0.7):
        return FetchResult(
            url=url,
            final_url="https://www.lemon8-app.com/discover/trending",
            http_status=200,
            ok=True,
            error_type=None,
            raw_html="""
    ...
    targets = [
        ValidationTarget(
            user_id="u1",
            url="https://s.lemon8-app.com/al/invalid",
            region="jp",
            expected_link_names=["f82_bmw4"],
        )
    ]
    rows = run_batch_validation(targets, allowed_regions={"jp"})
    assert rows[0]["read_count"] == 100
    assert rows[0]["effective_author_link_name"] is None
    assert rows[0]["author_source"] == "missing"
    assert rows[0]["ownership_status"] == "unknown"
    assert rows[0]["failure_reason"] == "author_missing"
